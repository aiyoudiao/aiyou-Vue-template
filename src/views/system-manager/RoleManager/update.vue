...
<script>
import {
    SYSTEM_ROLE_MANAGER_UPDATE,
    SYSTEM_ROLE_MANAGER_UPDATE_ZHCN,
...
const { UPDATE_API } = SystemRoleManager

export default {
    name: SYSTEM_ROLE_MANAGER_UPDATE,
    props: {
    ...
    data() {
        return {
            title: SYSTEM_ROLE_MANAGER_UPDATE_ZHCN,
            styles: {
    ...
    methods: {
        ...
        setFormData(state) {
            if (state) {
                ...
                this.$set(this.formData, 'roleId', roleId)
                this.$set(this.formData, 'roleName', roleName)
                this.$set(this.formData, 'roleName', roleName)
                this.$set(
                    this.formData,
                    'powerCode',
                    { guest: '0', admin: '1', administrator: '2' }[powerCode],
                )
                this.$set(this.formData, 'roleComments', roleComments)
                this.$set(this.formData, 'roleStatus', roleStatus)
            }
        },
        compareOldAndNewData() {
            ...
            if (roleId !== this.formData.roleId) {
                result = false
        ...
        && roleComments === this.formData.roleComments
            ) {
                result = false
